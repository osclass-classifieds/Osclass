  ...
  function doModel() {
    ...
    switch($this->action) {
      ...
      case('settings_post'):   // update item settings
        ...
        $enabledTinymceItems = Params::getParam('enabled_tinymce_items');
        $enabledTinymceItems = (($enabledTinymceItems == '1') ? true : false);
        ...
        $canDeactivateItems = Params::getParam('can_deactivate_items');
        $canDeactivateItems = (($canDeactivateItems != '') ? true : false);
        ...
        $itemSendFriendFormDisabled = Params::getParam('item_send_friend_form_disabled');
        $itemSendFriendFormDisabled = (($itemSendFriendFormDisabled != '') ? true : false);
        $regUserCanContact = Params::getParam('reg_user_can_contact');
        $regUserCanContact = (($regUserCanContact != '') ? true : false);
        $regUserCanSeePhone = Params::getParam('osc_reg_user_can_see_phone');
        $regUserCanSeePhone = (($regUserCanSeePhone != '') ? true : false);
        ...
        if(!osc_validate_int($warnExpiration) && $warnExpiration != -1) {
        ...
        $iUpdated += osc_set_preference('item_contact_form_disabled', $itemContactFormDisabled);
        $iUpdated += osc_set_preference('item_send_friend_form_disabled', $itemSendFriendFormDisabled);
        $iUpdated += osc_set_preference('reg_user_can_contact', $regUserCanContact);
        $iUpdated += osc_set_preference('reg_user_can_see_phone', $regUserCanSeePhone);
        ...
        $iUpdated += osc_set_preference('item_post_redirect', $itemPostRedirect);
