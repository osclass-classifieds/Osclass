...
class CAdminPlugins extends AdminSecBaseModel {
  ...
  function doModel() {
    ...
    // Run integrity check at start
    Plugins::integrityCheck();
    ...
    switch($this->action) {
      case 'bulk_actions':
        ...
        $plugins = array_filter(Params::getParam('id'));

        osc_run_hook('admin_before_plugin_bulk_action', $plugins, Params::getParam('bulk_actions'));
          ...
          switch(Params::getParam('bulk_actions')) {
            ...
            case 'enable_all':
              ...
              foreach($plugins as $p) {
                if(Plugins::isEnabled($p)) {
                  $numFailed++;
                  $msg[] = str_replace('/index.php', '', $p);

                } else if(!Plugins::isInstalled($p)) {
                  $numFailed++;
                  $msg[] = str_replace('/index.php', '', $p);

                } else {
                  if(Plugins::activate($p)) {
                    $numSuccess++;
            ...
            case 'delete_all':
              ...
              foreach($plugins as $p) {
                $pm = str_replace('/index.php', '', $p);
                $path = preg_replace('([\/]+)', '/', CONTENT_PATH.'plugins/'.$pm);

                if($pm !="" && strpos($pm, '../') === false && strpos($pm, '..\\') === false && $path != CONTENT_PATH.'plugins/') {
                  if(Plugins::deactivate($p) && Plugins::uninstall($p)) {
            ...
            case 'install_all':
              ...
              foreach($plugins as $p) {
                ...
                // header("Location: " . osc_admin_base_url(true) . "?page=plugins&error=" . $p, true, '302');

                if(Plugins::isInstalled($p)) {
                 $numFailed++;
                 $msg[] = str_replace('/index.php', '', $p);
                } else {
                  ...
                  if(isset($installed['error_code']) || $installed != 1) {
                    Plugins::deactivate($p);
                    $numFailed++;
                    $msg[] = str_replace('/index.php', '', $p);
                  } else {
                    $numSuccess++;
      ...
      case 'add_post':
        ...
        osc_run_hook('admin_before_plugin_add', $package, $status);
      ...
      case 'install':
        ...
        osc_run_hook('admin_before_plugin_install', $pn);
      ...
      case 'uninstall':
        ...
        osc_run_hook('admin_before_plugin_uninstall', Params::getParam("plugin"));
      ...
      case 'enable':
        ...
        osc_run_hook('admin_before_plugin_enable', Params::getParam("plugin"));
      ...
      case 'disable':
        ...
        osc_run_hook('admin_before_plugin_disable', Params::getParam("plugin"));
      ...
      case 'delete':
        ...
        osc_run_hook('admin_before_plugin_delete', $plugin);
      ...
      default:
        ...
        // filter out results in case search is active
        $pattern = trim(osc_esc_html(Params::getParam('sSearch')));

        if(Params::getParam('pluginSearchSubmit') == 1) {
          osc_set_session('pluginSearchTerm', $pattern);

        } else {
          $pattern = ($pattern != '' ? $pattern : osc_get_session('pluginSearchTerm'));
        }
        ...
        if($pattern <> '') {
          ...
          if(is_array($aPlugin) && count($aPlugin) > 0) {
            foreach($aPlugin as $plg) {
              $pInfo = osc_plugin_get_info($plg);
              $text = strtolower($pInfo['plugin_name'] . $pInfo['description'] . $pInfo['version'] . $pInfo['author']);
        ...
        // check if something was updated already
        if(is_array($aPluginsToUpdate) && count($aPluginsToUpdate) > 0) {
          foreach($aPlugin as $plg) {
            $info = osc_plugin_get_info($plg);

            if(isset($info['product_key']) && in_array($info['product_key'], $aPluginsToUpdate)) {
              $check = _need_update($info['product_key'], @$info['version'], true);       // using cached version of product versions for any price

              // plugin is in list of plugins to be updated, but actually is in latest version now
              if($check === false) {
                if(($key = array_search($info['product_key'], $aPluginsToUpdate)) !== false) {
                  unset($aPluginsToUpdate[$key]);
                }
              }
            }
          }
        ...
        if(is_array($aPluginsToUpdate)) {
          $aPluginsToUpdate = array_filter($aPluginsToUpdate);
        } else {
          $aPluginsToUpdate = array();
        }

        // maybe here, we should update back 'plugins_to_update' preference
        if(is_array($aPluginsToUpdate) && count($aPluginsToUpdate) > 0) {
          $bPluginsToUpdate = true;
        } else {
          $bPluginsToUpdate = false;
        }
        ...
        for($i = $start; $i < $max; $i++) {
          ...
          if($bPluginsToUpdate) {
            if(in_array(@$pInfo['product_key'],$aPluginsToUpdate )){
              $sUpdate = '<a class="market_update market-popup" href="#' . htmlentities($pInfo['product_key']) . '">' . __("Update") . '</a>';
              $sUpdateAlt = '<a class="market_update market-popup" href="#' . htmlentities($pInfo['product_key']) . '">' . __("New update available") . '</a>';
            }
          }
          ...
          $sSiteUrl = '';
          if($pInfo['plugin_uri']!='') {
            $sSiteUrl = ' | <a target="_blank" href="'. $pInfo['plugin_uri'] . '">'. __('Plugin details'). '</a>';
          }

          $sSupportUrl = '';
          if($pInfo['support_uri']!='') {
            $sSupportUrl = ' | <a target="_blank" href="'. $pInfo['support_uri'] . '">'. __('Support'). '</a>';
          }
          ...
          $row['bulkactions'] = '<input type="checkbox" name="id[]" value="' . $pInfo['filename'] . '" enabled="' . $enabled . '" installed="' . $installed . '"/>';
          $row['name'] = '<input type="hidden" name="installed" value="' . $installed . '" enabled="' . $enabled . '" /><div class="plg-tbl-top">' . $pInfo['plugin_name'] . '</div><div class="plg-tbl-bot plg-update-avl">' . $sUpdateAlt . '</div>';
          $row['description'] = '<div class="plg-tbl-top">' . $pInfo['description'] . '</div><div class="plg-tbl-bot">' . __('Version:') . ' ' . $pInfo['version'] . ' | ' . $sAuthor . $sSiteUrl . $sSupportUrl . '</div>';
          $row['update'] = ($sUpdate!='')   ? $sUpdate    : '&nbsp;';
          $row['configure'] = ($sConfigure!='')  ? $sConfigure   : '&nbsp;';
          $row['enable'] = ($sEnable!='')   ? $sEnable    : '&nbsp;';
          $row['install'] = ($sInstall!='')  ? $sInstall   : '&nbsp;';
          $row['delete'] = ($sDelete!='')   ? $sDelete    : '&nbsp;';
        ...
        $bulk_options = array(
          ...
          array('value' => 'enable_all', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected plugins?'), strtolower(__('Enable'))), 'label' => __('Enable')),
          array('value' => 'disable_all', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected plugins?'), strtolower(__('Disable'))), 'label' => __('Disable')),
          array('value' => 'install_all', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected plugins?'), strtolower(__('Install'))), 'label' => __('Install')),
          array('value' => 'uninstall_all', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected plugins?'), strtolower(__('Uninstall'))), 'label' => __('Uninstall')),
          array('value' => 'delete_all', 'data-dialog-content' => sprintf(__('Are you sure you want to %s the selected plugins?'), strtolower(__('Delete'))), 'label' => __('Delete'))
        );

        $bulk_options = osc_apply_filter("plugin_bulk_filter", $bulk_options);
