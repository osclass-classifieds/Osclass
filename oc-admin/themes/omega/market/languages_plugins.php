...
function customPageTitle($string) {
  return sprintf(__('%s on market - %s'), __('Plugins translations'), $string);
}
...
$products_json = osc_file_get_contents(osc_language_url('', $pattern, $sort, 'plugins'));
$products = json_decode($products_json, true);

$plugins_all_with_index = Plugins::listAll();
$plugins_all = array();
    ...
    <?php if(is_array($products) && count($products) > 0) { ?>
      ...
      <?php foreach($products as $p) { ?>
        <?php
          ...
          $pstat = 'NOT';
          if(in_array($prod_path, $plugins_all)) {
            $pstat = 'DOWNLOADED';
          }
...
<div id="market_installer" class="has-form-actions hide">
  <form name="mkti" action="<?php echo osc_admin_base_url(true); ?>?page=ajax&action=market&<?php echo osc_csrf_token_url(); ?>" method="post">
    <input type="hidden" name="market_type" value="plugins" />
...
<script>
  ...
  $(function() {
    ...
    $("#market_install").on("click", function(){
      ...
      $.getJSON(
      "<?php echo osc_admin_base_url(true); ?>?page=ajax&action=languages-plugins&<?php echo osc_csrf_token_url(); ?>",
      {"market_product_key" : $("#market_product_key").attr("value"), "market_download" : $("#market_download").attr("value"), "market_type": "plugins", "market_prod_name": $("#market_prod_name").attr("value"), "market_filename": $("#market_filename").attr("value")},
      function(data){
        var content = '';
