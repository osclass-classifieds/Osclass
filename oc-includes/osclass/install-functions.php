...
function _purify($value, $xss_check) {
  ...
  $_config->set('Cache.SerializerPath', $uploads_path);
...
function get_requirements() {
  ...
  if($php_min <> '') {
    $array['PHP min version'] = array(
      'requirement' => sprintf(__('PHP version %s or higher'), $php_min),
      'fn' => version_compare(PHP_VERSION, $php_min . '.0', '>='),
      'solution' => sprintf(__('At least PHP %s is recommended to run Osclass. You may talk with your hosting to upgrade your PHP version (optional).'), $php_min),
      'optional' => true
    );
  }
  ...
  if($php_max <> '') {
    $array['PHP max version'] = array(
      ...
      'fn' => version_compare(PHP_VERSION, $php_max . '.99', '<='),
      'solution' => sprintf(__('PHP %s is latest version supported by Osclass. You may talk with your hosting to downgrade your PHP version (optional).'), $php_max),
      'optional' => true
    );
  }

  $array['CURL extension for PHP'] = array(
  ...
  $array['MySQLi extension for PHP'] = array(
    ...
    'solution' => __('MySQLi extension is required. How to <a target="_blank" href="https://www.php.net/manual/en/mysqli.setup.php">install/configure</a>.')
  );

  $array['GD extension for PHP'] = array(
    'requirement' => __('GD extension for PHP'),
    'fn' => extension_loaded('gd'),
    'solution' => __('GD extension is required. How to <a target="_blank" href="https://www.php.net/manual/en/image.setup.php">install/configure</a>.')
  );

  $array['Folder <code>oc-content/uploads</code> exists'] = array(
  ...
  $array['Folder <code>oc-content/uploads</code> is writable'] = array(
  ...
  $array['Folder <code>oc-content/downloads</code> exists'] = array(
  ...
  $array['Folder <code>oc-content/downloads</code> is writable'] = array(
  ...
  $array['Folder <code>oc-content/languages</code> exists'] = array(
  ...
  $array['Folder <code>oc-content/languages</code> is writable'] = array(
...
function oc_install() {
  ...
  if($createdb) {
    ...
    $master_conn = new DBConnectionClass($dbhost, $adminuser, $adminpwd, '', true);
    $error_num = $master_conn->getErrorConnectionLevel();
    $error_desc = $master_conn->getErrorConnectionDesc();
    ...
    if($error_num > 0) {
      ...
      switch($error_num) {
        case 1049:
          ...
          break;
        case 1045:
          return array('error' => sprintf(__('Cannot connect to the database. Check if the DB user is correct and has required privileges. <br/>[%s: %s]'), $error_num, $error_desc));
          break;
        case 1044:
    ...
    $comm->query(sprintf("CREATE DATABASE IF NOT EXISTS %s DEFAULT CHARACTER SET 'utf8mb4' COLLATE 'utf8mb4_unicode_ci'", $dbname));    // Support for emoji
  ...
  $conn = new DBConnectionClass($dbhost, $username, $password, $dbname, true);
  $error_num = $conn->getErrorConnectionLevel();
  $error_desc = $conn->getErrorConnectionDesc();

  if($error_num == 0) {
    $error_num = $conn->getErrorLevel();
    $error_desc = $conn->getErrorDesc();
  }
...
function is_osclass_installed() {
  ...
  if(class_exists('DBConnectionClass')) {
    require_once ABS_PATH . 'config.php';     // Results in conflict on WEB_PATH constant that is already defined
      ...
      <div class="form-table">
        <table>
          ...
          <tr>
            ...
            <td><input type="text" id="dbhost" name="dbhost" value="localhost" size="25" required/></td>
          ...
          <tr>
            ...
            <td><input type="text" id="dbname" name="dbname" value="osclass" size="25" required/></td>
          ...
          <tr>
            ...
            <td><input type="password" id="password" name="password" value="" size="25" autocomplete="off" required/>
            </td>
        ...
        <div id="advanced_install" class="shrink">
          <div class="text">
            <span><?php _e('Advanced settings'); ?></span>
          </div>
        </div>
...
function display_target() {
  ...
  ?>
    ...
    <?php _e('A password will be automatically generated for you if you leave this blank.'); ?>
        ...
        <p class="space-left-25 left no-bottom loc-text"><?php _e('Choose a country where your target users are located'); ?>.</p>
        <div class="clear"></div>
        <div id="location">
...
function display_database_error($error, $step) {
  ?>
      <p class="flash error"><?php echo $error['error'] ?></p>
      <p class="margin25t"><a href="<?php echo get_absolute_url(); ?>oc-includes/osclass/install.php?step=<?php echo $step; ?>" class="btn btn-secondary"><?php _e('Go back'); ?></a></p>
...
function display_finish($password) {
  ...
  <div class="row cron-setup"><?php echo sprintf(__("Don't forget to setup %s and %s after installation."), '<a target="_blank" href="https://docs.osclass-classifieds.com/cron-setup-i82">CRON</a>', '<a target="_blank" href="https://docs.osclass-classifieds.com/configure-mail-server-i67">' . __('Mail server') . '</a>'); ?></div>

  <p class="margin25t buttons-complete">
    <a target="_blank" href="<?php echo get_absolute_url() ?>oc-admin/index.php" class="btn btn-primary"><?php _e('Go to backoffice'); ?></a>
