...
function osc_get_premiums($max = 2, $rand = false, $cache = true) {
  ...
  // Little hack to get alerts work in search layout
  $mSearch->reconnect();
  $premiums = $mSearch->getPremiums($max, $rand, $cache);
 ...
 * @return string
 */
function osc_premium_category_row() {
  return (array) osc_premium_field('category');
}
...
function osc_premium_category_description($locale = '') {
  ...
  return (string) osc_field(osc_premium_category_row(), 's_description', $locale);
}
...
function osc_premium_currency_symbol() {
  $aCurrency = osc_get_currency_row(osc_premium_currency());
