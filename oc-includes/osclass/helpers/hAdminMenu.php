...
function osc_draw_admin_menu() {
  ...
  $actual_full_url = osc_admin_base_url(true) . '?' . $actual_url;
  ...
  // Make plugin submenu item active. Only exact match.
  $plugins_out = str_replace('href="' . $actual_full_url . '"', 'class="active" href="' . $actual_full_url . '"', $plugins_out);
  ...
  foreach($aMenu as $key => $value) {
    ...
    if(array_key_exists('sub', $value)) {
      ...
      foreach($aSubmenu as $aSub) {
        ...
        if(!$is_moderator || ($is_moderator && $credential_sub === 'moderator')) { // show
          $url_submenu = $aSub[1];
          $url_submenu = str_replace(array(osc_admin_base_url(true) . '?' , osc_admin_base_url()) , '' , $url_submenu);
    ...
    // --- menu section
    $url_menu = $value[1];
    $url_menu = str_replace(array(osc_admin_base_url(true) . '?' , osc_admin_base_url()) , '' , $url_menu);
