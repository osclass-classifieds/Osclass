...
// Item url from item id only
function osc_item_url_by_id($item_id, $locale = '') {
  if($item_id <= 0) {
    return false;
  }
...
/**
 * Gets comment array from view and cache it if not exists
 *
 * @return array
 */
function osc_get_comment_row($id, $cache = true) {
  if($id <= 0) {
    return false;
  }
...
/**
* Gets comment reply array form view
*
* @return array $comment
*/
...
/**
* Gets a specific field from current comment reply
*
* @param type $field
* @param type $locale
* @return field_type
*/
function osc_comment_reply_field($field, $locale = '') {
  return osc_field(osc_comment_reply(), $field, $locale);
}
...
/**
 * Gets category row from current item
 *
 * @param string $locale
 * @return string
 */
function osc_item_category_row() {
  return (array) osc_item_field('category');
}
...
function osc_item_currency_symbol() {
  $aCurrency = osc_get_currency_row(osc_item_currency());
  return isset($aCurrency['s_description']) ? $aCurrency['s_description'] : '';
}
...
function osc_item_contact_name() {
  return (string) osc_apply_filter('item_contact_name', osc_item_field("s_contact_name"));
}

function osc_item_contact_phone($only_number = true) {
  $phone = osc_apply_filter('item_contact_phone', osc_item_field("s_contact_phone"));
  if(osc_reg_user_can_see_phone() == 1 && !osc_is_web_user_logged_in()) {
    if(strlen($phone) > 4) {
    ...
    if(!$only_number) {
      $phone = '<span title="' . osc_esc_html(__('You must login to see phone number')) . '">' . $phone . '</span>';
    }
  }
...
function osc_item_contact_other() {
  return (string) osc_apply_filter('item_contact_other', osc_item_field("s_contact_other"));
}
...
function osc_item_contact_email() {
  return (string) osc_apply_filter('item_contact_email', osc_item_field("s_contact_email"));
}
...
function osc_item_country() {
  if(osc_get_current_user_locations_native() == 1) {
    return (osc_item_field("s_country_native") <> '' ? osc_item_field("s_country_native") : osc_item_field("s_country"));
  }
...
function osc_item_region() {
  if(osc_get_current_user_locations_native() == 1) {
    return (osc_item_field("s_region_native") <> '' ? osc_item_field("s_region_native") : osc_item_field("s_region"));
  }
...
function osc_item_city() {
  if(osc_get_current_user_locations_native() == 1) {
    return (osc_item_field("s_city_native") <> '' ? osc_item_field("s_city_native") : osc_item_field("s_city"));
  }
...
function osc_item_show_phone() {
  return (bool) osc_item_field("b_show_phone");
}
...
/**
 * Gets if listing is eligible for renewal
 *
 * @param string $id
 * @return boolean
 */
...
/**
 * Check if user has not reached rating limit already
 *
 * @return boolean
 */
...
/**
 * Gets reply ID of current comment
 *
 * @return int
 */
function osc_comment_parent_id() {
  return (int) osc_comment_field("fk_i_reply_id");
}
 ...
 * @return array
 */
function osc_comment_replies() {
  return osc_comment_field("replies");
}

/**
 * Check if comment is reply to some other comment
 *
 * @return boolean
 */
function osc_comment_is_reply() {
  return (osc_comment_parent_id() === NULL ? true : false);
}
...
/**
 * Gets id of current comment reply
 *
 * @return int
 */
function osc_comment_reply_id() {
  return (int) osc_comment_reply_field("pk_i_id");
}

/**
 * Gets publication date of current comment reply
 *
 * @return string
 */
function osc_comment_reply_pub_date() {
  return (string) osc_comment_reply_field("dt_pub_date");
}

/**
 * Gets title of current comment reply
 *
 * @return string
 */
function osc_comment_reply_title() {
  return (string) osc_comment_reply_field("s_title");
}

/**
 * Gets author name of current comment reply
 *
 * @return string
 */
function osc_comment_reply_author_name() {
  return (string) osc_comment_reply_field("s_author_name");
}

/**
 * Gets author email of current comment reply
 *
 * @return string
 */
function osc_comment_reply_author_email() {
  return (string) osc_comment_reply_field("s_author_email");
}

/**
 * Gets body of current comment reply
 *
 * @return string
 */
function osc_comment_reply_body() {
  return (string) osc_comment_reply_field("s_body");
}

/**
 * Gets rating of current comment reply
 *
 * @return int
 */
function osc_comment_reply_rating() {
  return (int) osc_comment_reply_field("i_rating");
}

/**
 * Gets reply ID of current comment reply
 *
 * @return int
 */
function osc_comment_reply_parenty_id() {
  return (int) osc_comment_reply_field("fk_i_reply_id");
}


/**
 * Gets user id of current comment reply
 *
 * @return int
 */
function osc_comment_reply_user_id() {
  return (int) osc_comment_reply_field("fk_i_user_id");
}

/**
 * Gets  link to delete the current comment reply of current item
 *
 * @return string
 */
function osc_delete_comment_reply_url() {
  return (string) osc_base_url(true)."?page=item&action=delete_comment&id=".osc_item_id()."&comment=".osc_comment_reply_id()."&".osc_csrf_token_url();
}
//////////////////////////////
...
/**
 * Gets number of item comment replies
 *
 * @return int
 */
...
/**
 * Gets next comment reply of current item comment replies
 *
 * @return array
 */
function osc_has_comment_replies() {
  ...
  return View::newInstance()->_next('comment_replies');
}
...
function osc_query_item($params = null) {
  ...
  foreach($params as $key => $value) {
    switch($key) {
      ...
      case 'premium':
        $mSearch->withPicture($value == 1 ? true : false);
        break;
      ...
      case 'item_condition':
        $mSearch->addItemConditions($value);      // can be array or one string
        break;
