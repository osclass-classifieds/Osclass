...
class PageForm extends Form {
   ...
   * @param null $page
   */
  public static function pub_date_input_text($page = null) {
    $pubDate = '';
    if(is_array($page) && isset($page['dt_pub_date']) && trim((string)$page['dt_pub_date']) != '') {
      $pubDate = $page['dt_pub_date'];
    }

    if(Session::newInstance()->_getForm('dt_pub_date') != '') {
      $pubDate = Session::newInstance()->_getForm('dt_pub_date');
    }

    parent::generic_input_text('dt_pub_date', $pubDate, null, false, true, -1, 'text', 'yyyy-mm-dd hh:mm:ss');
  }

  // Position/order input for orderable static pages
  public static function order_input_text($page = null, $isNew = false) {
    $pageManager = Page::newInstance();
    $min = 1;

    if($isNew) {
      $max = $pageManager->getNextPositionableOrder();
      $order = $max;
    } else {
      $max = max(1, $pageManager->getMaxPositionableOrder());
      $order = 1;
      if(is_array($page) && isset($page['i_order'])) {
        $order = max(1, (int)$page['i_order']);
      }
    }

    if(Session::newInstance()->_getForm('i_order') != '') {
      $order = max(1, (int)Session::newInstance()->_getForm('i_order'));
    }
    ...
    $name = 'i_order';
    echo '<input id="' . preg_replace('|([^_a-zA-Z0-9-]+)|', '', $name) . '" type="number" name="' . osc_esc_html($name) . '" value="' . (int)$order . '" min="' . (int)$min . '" max="' . (int)$max . '" step="1" />';
  }
