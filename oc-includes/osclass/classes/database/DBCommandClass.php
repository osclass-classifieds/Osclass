...
class DBCommandClass
{
  ...
  /**
   * Create the UPSERT sql and perform the query (insert, on duplicated key update)
   *
   * @access public
   *
   * @param mixed $table
  ...
  public function upsert($table = '', $set = null, $where = null) {
    ...
    $sql = $this->_upsert($table, $this->aSet, $this->aWhere);
  ...
  public function importSQL($sql) {
    ...
    // Force ENGINE=InnoDB (case-insensitive)
    ...
    if(defined('IMPORTSQL_FORCE_COLLATE') && IMPORTSQL_FORCE_COLLATE != '') {
      ...
      $sql = preg_replace($collatePattern, 'COLLATE=' . IMPORTSQL_FORCE_COLLATE, $sql);
    }
...
//              error_log(' --- ' . $q );
  ...
  /**
   * Run or print sql based on param
   *
   * @access public
   * @return string
   * @since  8.3.1
  ...
  /**
   * Convert database, tables, columns into utf8mb4
   *
   * @access public
   * @return string
   * @since  8.3.1
   */
  public function migrateToUtf8mb4($dryRun = false) {
    $charset = 'utf8mb4';
    ...
    // Disable FK checks
    $this->_runOrPrint('SET FOREIGN_KEY_CHECKS = 0', $dryRun);

    // Convert database default charset/collation
    $dbName = DB_NAME;
    $this->_runOrPrint("ALTER DATABASE `$dbName` CHARACTER SET $charset COLLATE $collate", $dryRun);
      ...
      foreach($tables as $t) {
        ...
        if(is_array($fks) && count($fks) > 0) {
          foreach($fks as $fk) {
            ...
            $this->_runOrPrint("ALTER TABLE `$table` DROP FOREIGN KEY `$name`", $dryRun);
          }
        }
        ...
        // --- 3) Convert only text columns still not utf8mb4 ---
        $cols = $this->query("
          SELECT COLUMN_NAME, COLUMN_TYPE, IS_NULLABLE, COLUMN_DEFAULT
          FROM information_schema.COLUMNS
          WHERE TABLE_SCHEMA = DATABASE()
            AND TABLE_NAME = '$table'
            AND (CHARACTER_SET_NAME <> '$charset' OR COLLATION_NAME <> '$collate')
    ...
    // Re-enable FK checks
    $this->_runOrPrint('SET FOREIGN_KEY_CHECKS = 1', $dryRun);
