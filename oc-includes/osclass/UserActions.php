...
class UserActions {
  ...
  public function add() {
    ...
    if($input['s_username'] == '') {
      if(osc_username_generator() == 'SLUG') {
        ...
        $this->manager->update(array('s_username' => $generate_username), array('pk_i_id' => $userId));
      } else {
  ...
  public function edit($userId) {
    ...
    $input = osc_apply_filter('user_edit_data', $input);
  ...
  public function recover_password() {
    ...
    if((osc_recaptcha_enabled() && osc_recaptcha_private_key() != '') && Session::newInstance()->_get('recover_captcha_not_set') != 1 && !osc_check_recaptcha()) {
  ...
  public function prepareData($is_add) {
    ...
    //locations...
    $country = osc_get_country_row(Params::getParam('countryId'));
    ...
    if((int)Params::getParam('regionId') > 0) {
      $region = osc_get_region_row(Params::getParam('regionId'));
    } else {
    ...
    if((int)Params::getParam('cityId') > 0) {
      $city = osc_get_city_row(Params::getParam('cityId'));
    } else {
