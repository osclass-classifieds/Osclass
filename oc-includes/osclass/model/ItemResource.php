...
class ItemResource extends DAO {
  ...
  public function __construct() {
    ...
    $this->setFields(array('pk_i_id', 'fk_i_item_id', 's_name', 's_extension', 's_content_type', 's_path', 'i_order'));
  }
  ...
  /**
   * Get all resources for regenerate process
   *
   * @access public
   * @since  unknown
   * @return array of resources
   */
  public function getAllResourcesFromId($last_id = 0) {
    $this->dao->select();
    $this->dao->from($this->getTableName());

    $last_id = (int)$last_id;
    if($last_id > 0) {
      $this->dao->where('pk_i_id > ' . $last_id);
    }
  ...
  public function getAllResourcesFromItem($itemId) {
    ...
    if($cache === false) {
      ...
      $this->dao->orderBy('i_order ASC, pk_i_id', 'ASC');
  ...
  public function getResources($itemId = NULL, $start = 0, $length = 10, $order = 'r.pk_i_id', $type = 'DESC') {
    $allowed_sort = array('r.pk_i_id', 'r.i_order ASC, r.pk_i_id', 'r.s_name', 'r.s_extension', 'r.s_content_type', 'r.s_path', 'r.i_order', 'r.fk_i_item_id', 'c.dt_pub_date');

    if(!in_array($order, $allowed_sort)) {
      return array();
    }

    if(!in_array(strtoupper($type), array('DESC', 'ASC', 'RAND()'))) {
