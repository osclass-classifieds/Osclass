...
*/
  ...
  public function refreshNumComments() {
    $sql = sprintf('UPDATE %s as u SET u.i_comments = (SELECT count(*) FROM %s as c WHERE c.fk_i_user_id = u.pk_i_id AND c.b_active = 1 AND c.b_spam = 0 AND c.b_enabled = 1) WHERE u.b_active = 1', $this->getTableName(), DB_TABLE_PREFIX . 't_item_comment');
    return $this->dao->query($sql);
  }
