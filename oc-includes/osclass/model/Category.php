...
class Category extends DAO
{
  ...
  /**
   * Function to search different types of conditions
   * Just enter your SQL conditions like "b.s_name != '' AND a.b_enabled = 1"
  ...
  /**
   * Returns tree of a given category as the root
   *
   * @access public
   * @since unknown
  ...
  public function updateByPrimaryKey($data, $pk) {
    ...
    if($res >= 0) {
      ...
      // update dt_expiration (table t_item) using the max date value
        ...
        $update_dt_expiration = sprintf("update %st_item as a set a.dt_expiration = '9999-12-31 23:59:59' where a.fk_i_category_id = %s", DB_TABLE_PREFIX, $pk);
        $this->dao->query($update_dt_expiration);
      }
